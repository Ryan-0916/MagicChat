...
public class MessageHistoryStorage {
     ...
     * 撤回渠道消息
     * @param channel 渠道
     * @param messageRetract 渠道消息
     */
    public void retractChannelMessage(Channel channel, MessageRetract messageRetract) {
        /* ToDo 待优化 不安全的线程问题 */
        ...
        for (ChannelMessage msg : channelStorage.getMessages(channel.getChannelName())) {
            if (msg.getMessageId().equals(messageRetract.messageId())) {
                msg.setRetracted(true);
                msg.setRetractedBy(messageRetract.receivedBy());
                break;
            }
        }
    }
